...
ReactTransitionGroup = ReactAddons.TransitionGroup

SpeechMixin = require './react-speech-based-mixin.js'
...
Demo = React.createClass({
  mixins: [SpeechMixin]
  intentActionMap:
    'set_temperature': '_showThermostat'
  _showThermostat:(message, data) ->
      temp = data[0].entities?.temperature[0]?.value
      unit = data[0].entities?.temperature[0]?.unit
      if not temp
